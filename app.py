...
if submitted:
    ...
    st.info("スコアに基づき、コンサルタント視点で推奨される「認知ハック」と「具体的なアクション」を提示します。")
    ...
    if s_exp_int <= 12:
        recommendations.append({
            ...
            "problem": "「今」に集中するあまり、未来の利益を過小評価しやすい傾向があります。",
            "methods": [
                {
                    "name": "Time Boxing (タイムボクシング)",
                    "how_to": "Googleカレンダー等で、タスクの「開始」と「終了」時間をブロックし、その時間は絶対にその作業以外しないと決める。",
                    "tips": "「できればやる」ではなく「会議」のように扱うのがコツ。最初は30分単位から。"
                },
                {
                    "name": "Unpacking (アンパッキング)",
                    "how_to": "「企画書作成」などの大きなタスクを、「ファイル作成」「目次案出し」「導入部執筆」など5分で終わるレベルまで分解する。",
                    "tips": "面倒だと感じるのはタスクが大きすぎるから。分解すれば脳は「これならできる」と認識する。"
                }
            ]
        })
    ...
    if s_exp_int >= 13:
        recommendations.append({
            ...
            "problem": "未来のリスクを過大評価し、プレッシャーや不安を感じやすい傾向があります。",
            "methods": [
                {
                    "name": "Pre-commitment (プレコミットメント)",
                    "how_to": "数ヶ月先に「キャンセル不可能な休暇（航空券予約など）」や「遊びの予定」を先に入れてしまう。",
                    "tips": "意志の力で休むのは不可能と割り切る。環境によって強制的に休む状況を作る。"
                },
                {
                    "name": "Functional Alibi (機能的アリバイ)",
                    "how_to": "「良い仕事をするために、今は脳を休める必要がある」と、休息に論理的な正当性を与える。",
                    "tips": "サボるのではなく「メンテナンス業務」と定義し直す。"
                }
            ]
        })
        ...
        recommendations.append({
            ...
            "methods": [
                ...
                },
                {
                    ...
                    "tips": "「価値の低い仕事」を特定し、勇気を持って捨てるか、質を下げる許可を自分に出す。"
                }
            ]
        })
    ...
    if s_rec_acc <= 12:
        recommendations.append({
            ...
            "methods": [
                ...
                {
                    "name": "Reference Class Forecasting (外部視点の導入)",
                    "how_to": "自分の予測ではなく、過去の類似プロジェクトの実績値や、他人の所要時間を基準に見積もる。",
                    "tips": "「自分は特別だ」という思い込みを捨て、データだけを見る。"
                }
            ]
        })
            ...
            "methods": [
                {
                    ...
                    "how_to": "「朝起きられた」「メールを1通返した」など、些細な達成事項を毎日3つ書き留める。",
                    "tips": "大きな成果ではなく「できた事実」を脳に刷り込むことが目的。"
                },
                {
                    "name": "Negative Simulation Check (予実管理)",
                    "how_to": "作業前に予想した「辛さ(1-10)」と、作業後の「実際の辛さ」を比較記録する。",
                    "tips": "多くの場合「案ずるより産むが易し」であることにデータで気づく。"
                }
            ]
        })
    ...
    st.markdown("---")
    st.caption("Reference: 『YOUR TIME ユア・タイム』(鈴木 祐 著) / Prototype by Your Name")
